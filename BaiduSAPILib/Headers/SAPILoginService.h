 ...
 *  @param bindType        与第三方帐号的绑定类型：implicit－暗绑，explicit－明绑，optional－选择性绑定、sms－明绑且只支持通过手机号绑定
 ...
 *  @param errorBlock      失败时回调
 *
 *  @return 传入username为空时返回NO，其他返回YES
 */
 ...
 *  @param errorBlock      失败时回调
 *
 *  @return 传入newUser为NO，bduss为空时返回NO，其他返回YES
 */
